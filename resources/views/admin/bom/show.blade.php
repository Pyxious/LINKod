...
@section('page-title', 'Review & Manage List of Materials')
    ...
    <div class="bg-[#fffde7] dark:bg-[#1c1c1e] border-2 border-[#0033a0] dark:border-blue-600 rounded-2xl px-6 sm:px-8 py-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                List of Materials Management & Approval
            </h1>
...
@php
    $nonEditableStatuses = ['Cancelled', 'Rejected'];
...
@endphp
    ...
    <!-- Main Materials Table Form Area -->
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-gray-100 dark:border-zinc-800">
            <div>
                <h2 class="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    <span>Requested Materials & Quantities</span>
                </h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    Review requested items and manage the list of materials for this project.
                </p>
            </div>
            ...
            <form action="{{ route('admin.bom.approve', $project->project_id) }}" method="POST" @submit="submitting = true">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-zinc-800 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider">
                                ...
                                <th class="py-3 px-3 text-center w-32">Qty</th>
                                <th class="py-3 px-3 text-center w-32">Unit</th>
                                <th class="py-3 px-3 text-center w-32">Status</th>
                                <th class="py-3 px-3 text-center w-20">Action</th>
                            </tr>
                        </thead>
                            ...
                            <template x-for="(item, idx) in items" :key="item.bom_id">
                                <tr class="hover:bg-blue-50/40 dark:hover:bg-zinc-800/40 transition">
                                    ...
                                    <td class="py-3.5 px-3 text-center">
                                        @if($isBomEditable)
                                            <button type="button" 
                                                    @click="if(confirm('Remove this material from the list?')) { document.getElementById('delete-bom-' + item.bom_id).submit(); }" 
                ...
                <div class="mt-6 pt-5 border-t border-gray-200 dark:border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50 dark:bg-zinc-800/50 p-5 rounded-2xl">
                    <div class="flex items-center gap-2">
                        <span class="text-sm font-bold text-slate-700 dark:text-gray-300">Total Listed Items:</span>
                        <span class="text-base font-black text-[#0033a0] dark:text-blue-400" x-text="items.length + ' item(s)'"></span>
                    </div>
                    ...
                    @if($isBomEditable)
                        <button type="submit" 
                            ...
                            <span x-text="submitting ? 'Saving...' : 'Verify & Forward to Client for Approval'">Verify & Forward to Client for Approval</span>
                        </button>
                    @endif
                </div>
            </form>
    ...
    <div class="bg-white dark:bg-[#1c1c1e] rounded-2xl border border-gray-200 dark:border-zinc-800 p-6 sm:p-7 shadow-sm"
         x-data="{
             ...
             unit: 'pcs',
             qty: 1,
             catalog: {{ Js::from($allMaterials->map(fn($m) => ['id' => $m->material_id, 'name' => $m->material_name, 'unit' => $m->unit_of_measurement ?? 'pcs'])) }},
             isDiscrete(unit) {
                 if (!unit) return true;
                 const u = unit.toString().trim().toLowerCase();
                 const continuousUnits = ['meter', 'meters', 'm', 'length', 'lengths', 'ft', 'feet', 'foot', 'liter', 'liters', 'l', 'kg', 'kilo', 'kilos', 'kilogram', 'kilograms', 'gallon', 'gallons', 'gal', 'yard', 'yards', 'yd', 'inch', 'inches', 'cm', 'mm'];
