        ...
        <div class="fixed top-20 right-4 sm:right-6 z-50 pointer-events-none w-full max-w-sm sm:max-w-md space-y-3">
            @if(session('success'))
                <div x-data="{ show: true }" 
                    ...
                    <div class="flex items-center gap-2.5 flex-1 min-w-0">
                        <svg class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                        <span class="leading-snug break-words">{{ session('success') }}</span>
                    </div>
                    <button type="button" @click="show = false" class="text-emerald-700 hover:text-emerald-950 dark:text-emerald-300 dark:hover:text-white p-1 rounded-md transition cursor-pointer text-xs font-bold leading-none shrink-0" title="Dismiss">
                        ✕
                    </button>
                </div>
            @endif
            @if(session('error'))
                <div x-data="{ show: true }" 
                    ...
                    <div class="flex items-center gap-2.5 flex-1 min-w-0">
                        <svg class="w-5 h-5 text-red-600 dark:text-red-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                        <span class="leading-snug break-words">{{ session('error') }}</span>
                    </div>
                    <button type="button" @click="show = false" class="text-red-700 hover:text-red-950 dark:text-red-300 dark:hover:text-white p-1 rounded-md transition cursor-pointer text-xs font-bold leading-none shrink-0" title="Dismiss">
                        ✕
                    </button>
                </div>
            @endif
