...
<div id="messages-section" class="bg-white dark:bg-[#1c1c1e] rounded-2xl border border-gray-200 dark:border-zinc-800 p-6 shadow-2xs font-sans mt-6">
    ...
    <!-- Messages Container -->
    <div class="space-y-4 max-h-96 overflow-y-auto pr-1 mb-5 scroll-smooth">
    ...
    @else
        <form action="{{ $postRoute }}" method="POST" enctype="multipart/form-data" class="space-y-3">
            @csrf
            <div class="relative">
                <textarea name="message" rows="3" placeholder="Type a message or note for this requisition..." class="w-full p-3.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs text-slate-800 dark:text-gray-200 focus:outline-none focus:border-[#0038A8] transition resize-none" required></textarea>
            </div>
