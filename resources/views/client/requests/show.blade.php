...
@section('content')
<div class="w-full max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6 font-sans">
    
    <!-- Hero Header Banner -->
    <div class="bg-white dark:bg-[#1c1c1e] border-2 border-[#0033a0] dark:border-blue-600 rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <!-- Breadcrumb -->
            ...
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                {{ $request->title }}
            </h1>
            <p class="text-xs text-gray-500 mt-1">Submitted on {{ \Carbon\Carbon::parse($request->submitted_at)->format('F d, Y \a\t h:i A') }}</p>
        </div>

        <!-- Action Buttons -->
        <div class="flex items-center gap-3 shrink-0">
            @if($request->current_status === 'Completed' && !$request->evaluation)
                <a href="{{ route('client.evaluations.create', $request->request_id) }}" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition shadow-sm inline-flex items-center gap-2">
            ...
            @if(in_array($request->current_status, ['Submitted', 'Pending']))
                <form action="{{ route('client.requests.cancel', $request->request_id) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this request?');">
                    @csrf
                    <button type="submit" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl transition shadow-sm inline-flex items-center gap-2">
                    ...
                    if (history) {
                        ...
                        if (timeline) {
                            ...
                            item.innerHTML = `
                                ...
                                <div class="bg-blue-50/60 dark:bg-zinc-800/60 p-3.5 rounded-xl border border-blue-200 dark:border-zinc-700">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-xs font-extrabold text-[#0033a0] dark:text-blue-400">
